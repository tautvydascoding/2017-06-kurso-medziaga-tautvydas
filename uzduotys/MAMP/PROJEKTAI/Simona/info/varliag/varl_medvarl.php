<!DOCTYPE html>
<html lang="lt">

<head>
  <meta charset="utf-8">

  <title> LRK </title>
  <meta name="description" content="Lietuvos Raudonoji knyga, itin reti Lietuvos augalai, gyvūnai, grybai" />
  <meta name="author" content="Simona Matiukaite" />
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Open+Sans|Spectral:300i">
  <link rel="stylesheet" type="text/css" href="../../bootstrap1/css/bootstrap.css" />
  <link rel="stylesheet" type="text/css" href="../../bootstrap1/js/jquery-3.2.1.min.js" />
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
  <link rel="stylesheet" type="text/css" href="../../bootstrap1/css/main.css" />
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
  <link rel="stylesheet" type="text/css" href="../../bootstrap1/js/main.js" />


</head>

<body>
  <header>

    <!-- Menu icons -->
    <nav class="navbar navbar-inverse navbar-fixed-top" >
      <div class="container-fluid menu" >
        <div class="navbar-header">
          <a class="navbar-brand" href="../../index.php">Lietuvos Raudonoji knyga</a>
        </div>
        <ul class="nav navbar-nav">
           <li title="Pradinis puslapis"><a href="../../index.php" target="">
            <img src="../../icons/home.png" alt="namai" width="40" height="40" />
            <div class="namai">
            </div>
          </a></li>

          <li title="Apie..."><a href="#Apie" target="">
            <img src="../../icons/apie.png" alt="apie" width="38" height="38" />
          </a></li>

          <li title="Galerija"><a href="#Galerija" target="">
              <img src="../../icons/gallery.png" alt="galerija" width="40" height="40"/>
            </a></li>

          <li title="Susisiekime"><a href="#Susisiekime" target="">
              <img src="../../icons/mail.png" alt="pastas" width="40" height="40" />
            </a></li>

            <li title="Gyvūnai"><a href="../../search_animals.php" target="">
                <img src="../../icons/ipaw70.png" alt="gyvunai" width="40" height="40" />
              </a></li>

            <li title="Augalai"><a href="../aug/aa_bekotis.php" target="">
                <img src="../../icons/ileaf70.png" alt="Augalai" width="40" height="40" />
                </a></li>

            <li title="Grybai"><a href="../gryb/ag_baco.php" target="">
                <img src="../../icons/imushroom75.png" alt="Grybai" width="40" height="40" />
                </a></li>

        </ul>
        <form action="" method="post" class="navbar-form navbar-left">
          <div class="form-group">
          <input type="text" name="name" class="form-control" placeholder="Paieška" />
                </div>
                <button type="submit" class="btn btn-default">Tvirtinu</button>
              </form>
            </div>
          </nav>

    <div class="container-fluid pirmas" style="margin-top:70px">
<section>
      <div class="row1">

        <!-- RODYKLINIS MENIU -->
          <div class="col-xs-1">
            <div class="row-frst">
              <ul>
                <h4><ins> Varliagyviai </ins></h4><br>
                <li><a href="varl_medvarl.php"><h5>Europinė medvarlė</h5>
                </a></li><br>
                <li><a href="avar_rupuz.php"><h5>Žalioji rupūžė</h5>
                </a></li><br>




            </ul>
            </div>
          </div>

  <!-- APRASYMAS IS DUOMENU BAZES  -->
          <div class="col-xs-9">
            <div class="remelis2">
              <div class="row-snd">
                <div class="col-xs-4">
                  <div class="article">
                    <h4>Europinė medvarlė</h4>
                    <h5><i> Hyla arborea </i></h5><br /><br />
                    <p>
Statusas. ¡ Lietuvos raudon‡j‡ knyg‡ r ̊ is ·ra yta nuo 2000 m.
¡ra yta · Berno konvencijos II ir ES Buveini ̄ direkty- vos IV priedus.
Paplitimas. Vidurio ir Piet ̄ Europoje · rytus iki Kau- kazo, Turkijoje. –iaurinÎ arealo riba ñ Danija, –vedi- ja, Lietuva, Baltarusija.
Lietuvoje r ̊ is aptinkama lokaliai (Lazdij ̄, VarÎnos r., Druskinink ̄ sav.). Duomenys apie radavietÊ Ma ̨eiki ̄ r. nepatvirtinti. 1928 m. nurodyta radavietÎ Vilniuje.
Biologija ir ekologija. Gyvena lapuoËi ̄, mi ri ̄ mi k ̄, ·vairi ̄  ̨eldini ̄ pakra Ëiuose, pievelÎse prie vandens telkini ̄. Pirmieji i   ̨iemovieËi ̄ pasirodo patinai (ba- land ̨io pabaigoje). ¡ ner tavietes varliagyviai susirenka baland ̨io pabaigojeñgegu ̨Îs mÎn. Ner tavietÎs ñ ge- rai saulÎs · ildomi vandens telkiniai, k ̊dros prie mi - keli ̄ su gausia vandens ir pakrantÎs  ̨oline augalija. PatelÎs i ner ia apie 800ñ1 000 ikreli ̄, porcijomis, ne- dideliais gumuliukais, ir priklijuoja juos prie dugno augalijos. Buo ̨galviai baigia metamorfozÊ ir i lipa · krant‡ antroje liepos pusÎjeñrugpj ̊Ëio mÎn. Gyvena pavie- niui, da ̨niausiai t ̊no ant ·vairi ̄  ̨em ̄ kr ̊mok ni ̄, auk - t ̄  ̨oli ̄. Minta ·vairiais bestuburiais, daugiausia vabz- d ̨iais. fiiemoja sausumoje, ·vairiuose urveliuose, ply-  iuose, ·sirausia po nukritusiais lapais.
Populiacij ̄ gausumas. PagrindinÎs populiacijos yra Bu- giedoje (2ñ5 ind./m  suaugÎli ̄ veisimosi metu, 5ñ7  i ̄metukai 1 m  po metamorfozÎs), Gerda iuose, Kap- Ëiamiestyje. Populiacij ̄ gausumas skirtingais sezonais priklauso nuo temperat ̊ros, hidrologini ̄ pokyËi ̄. Lie- tuvos populiacija vertinama 1 500ñ2 000 gyv ̊n ̄.
                      </p>
                  </div>
                </div>
                <div class="col-xs-4">
                  <div class="article">
                    <p>
                      Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor
                      in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.
                    </p>
                  </div>
                </div>
                <div class="col-xs-4">
                  <div class="remelis3">
                    <!-- NUOTRAUKA IS DUOMENU BAZES -->
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>


      </div>
    </div>

  </header>
</body>

<footer>
  <div class="container-fluid footeriukas">
    <div class="row-foot">
        <div class="foot-textukas">
          <h7> &#169 2017m. - Visos teisės saugomos </h7>
        </div>
      </div>
    </div>
  </div>
</footer>

</html>
